...
var hasSelection = window.getSelection ? function(te) {
  ...
  try {var range$$1 = te.ownerDocument.selection.createRange()}
  catch(e) {}
...
function nodeAndOffsetInLineMap(map$$1, ch, bias) {
  ...
  for (var i = 0; i < map$$1.length; i += 3) {
    var mStart = map$$1[i], mEnd = map$$1[i + 1]
    if (ch < mStart) {
      start = 0; end = 1
      collapse = "left"
    } else if (ch < mEnd) {
...
function leftButtonSelect(cm, e, start, type, addNew) {
  ...
  function extendTo(pos) {
    ...
    if (type == "rect") {
      ...
      if (type != "single") {
        if (type == "double")
          var range$$1 = cm.findWordAt(pos)
        else
          var range$$1 = new Range(Pos(pos.line, 0), clipPos(doc, Pos(pos.line + 1, 0)))
